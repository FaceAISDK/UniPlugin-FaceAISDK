...
open class ResultJSON (
    ...
    override fun `__$getOriginalPosition`(): UTSSourceMapPosition? {
        return UTSSourceMapPosition("ResultJSON", "uni_modules/FaceAI-Search/utssdk/interface.uts", 5, 13)
    }
}
...
val insertFaceSearchFeature: InsertFaceSearchFeature = fun(faceID: String, faceFeature: String, tag: String, group: String, callback: (result: ResultJSON) -> Unit) {
    ...
    if (faceID == null || faceID.trim() == "") {
        val errorResult = ResultJSON(code = -1, msg = "参数错误: faceID 不能为空")
        console.error(errorResult.msg, " at uni_modules/FaceAI-Search/utssdk/app-android/index.uts:73")
    ...
    if (faceFeature == null || faceFeature.length != 1024) {
        ...
        val errorResult = ResultJSON(code = -2, msg = "参数错误: faceFeature 长度必须为 1024 (当前长度: " + currentLen + ")")
        console.error(errorResult.msg, " at uni_modules/FaceAI-Search/utssdk/app-android/index.uts:85")
        callback(errorResult)
        return
    }
    try {
        ...
        val successResult = ResultJSON(code = 1, msg = "success")
        callback(successResult)
    }
     catch (e: Exception) {
        val catchResult = ResultJSON(code = -3, msg = "Native 执行异常: " + e.message)
        console.error("insertFaceSearchFeature error: " + e.message, " at uni_modules/FaceAI-Search/utssdk/app-android/index.uts:114")
...
val insertManyFeatures: InsertManyFeatures = fun(jsonFaceFeatures: String, callback: (result: ResultJSON) -> Unit) {
    ...
    val resultJson = ResultJSON(code = faceCount, msg = "insertManyFeatures")
    callback(resultJson)
}
val deleteFaceSearchFeature: DeleteFaceSearchFeature = fun(faceID: String, callback: (result: ResultJSON) -> Unit) {
    ...
    val resultJson = ResultJSON(code = 1, msg = "delete success")
    callback(resultJson)
}
    ...
    UTSAndroid.onAppActivityResult(fun(requestCode: Int, resultCode: Int, intentAct: Intent?){
        if (requestCode == 10086) {
            if (intentAct != null) {
                ...
                val resultJson = ResultJSON(code = codeNow, msg = msgNow)
                console.log("添加人脸人脸：" + resultJson, " at uni_modules/FaceAI-Search/utssdk/app-android/index.uts:207")
                callback(resultJson)
            } else {
                val resultJson = ResultJSON(code = -1, msg = "添加失败")
                callback(resultJson)
            }
        }
    }
